...
const Timeline = ({ itinerary, className, style }) => {
    return (
        <div className={`w-full ${className}`} style={style}>
            <h2 className="text-3xl mb-12 text-rust">Itinerary</h2>
                        ...
                        return (
                            <div key={item.dayNumber || index} className="relative px-4  pb-16 last:pb-0">
                                ...
                                <div className="-mt-1.5">
                                    <span className="text-xs uppercase tracking-[0.2em] text-rust block mb-2">Day {item.dayNumber}</span>
                                    <h3 className="text-2xl mb-4 text-rust">{item.title}</h3>
                                    {item.description && <p className="text-zinc-600 leading-relaxed text-sm mb-6">{item.description}</p>}

                                    {item.activities && item.activities.length > 0 && (
                                        <div className="bg-paper p-3 md:p-4 lg:p-6 rounded-md">
                                            <p className="text-xs uppercase tracking-widest text-rust mb-1">Activities</p>
                                            <ul className="flex flex-col gap-0.5">
                                                    ...
                                                    <li
                                                        ...
                                                        <span className="text-xs">
                                                            ...
                                                            {activity.isExtraCharge ? (
                                                                <span className="text-red-600 text-xs">(+₹{activity.extraChargeAmount})</span>
                                                            ) : (
                                                                ""
                                                            )}
                                                        </span>
                                                ...
                                                <p className="text-xs uppercase tracking-widest text-rust">Additional Info</p>
                                                {item.mealsIncluded && (
